...
export async function renderSettings(root, tab) {
  ...
  const tabs = [['account', 'Account'], ['claude', 'AI configuration'], ['niches', 'Niche preferences'], ['preferences', 'Lead generation'], ['notifications', 'Notifications']];
...
async function claude(body, s) {
  const c = s.ai || s.claude;
  const render = (c2) => {
    const label = c2.provider_label || (c2.provider === 'gemini' ? 'Google Gemini' : 'Anthropic Claude');
    const keyHint = c2.key_hint || '';
    body.innerHTML = html`<div class="grid grid-2"><div class="card"><div class="card-head"><h2>${label} API configuration</h2>${raw(badge(c2.active_source === 'none' ? 'Not configured' : c2.active_source === 'user' ? 'Using your key' : 'Using server key', c2.active_source === 'none' ? 'danger' : 'success'))}</div>
      <p class="small muted">The active AI provider is <b>${label}</b> (set by the administrator via AI_PROVIDER). Keys are stored encrypted on the server and never sent back to the browser. If you save a personal key it is used for your lead generation and research; otherwise the server-wide key is used.</p>
      <dl class="kv"><dt>Your key</dt><dd>${c2.has_user_key ? raw(`•••• ${esc(c2.user_key_last4)} ${badge(c2.user_key_status, c2.user_key_status === 'valid' ? 'success' : c2.user_key_status === 'invalid' ? 'danger' : 'neutral')}`) : 'none saved'}</dd><dt>Server key</dt><dd>${c2.server_key_configured ? 'configured' : 'not configured'}</dd><dt>Model</dt><dd>${c2.model}${c2.fallback_models && c2.fallback_models.length ? raw(`<span class="sub">fallbacks: ${esc(c2.fallback_models.join(', '))}</span>`) : raw('')}</dd><dt>Web research</dt><dd>${c2.web_search ? `enabled (${c2.search_label || 'live web search'}${c2.url_context ? ' + URL reading' : ''})` : 'disabled'} · business-data provider: ${c2.search ? c2.search.provider : 'none'}${c2.search && c2.search.provider !== 'none' && !c2.search.enabled ? ' (key missing)' : ''}</dd>${c2.thinking ? html`<dt>Thinking</dt><dd>${c2.thinking}</dd>` : raw('')}<dt>Encryption</dt><dd>${c2.encryption_configured ? 'APP_ENCRYPTION_KEY set' : raw('<span class="badge danger">APP_ENCRYPTION_KEY missing — personal keys cannot be saved</span>')}</dd><dt>Last test</dt><dd>${c2.last_tested_at ? `${fmtDateTime(c2.last_tested_at)} · ${c2.last_test_result && c2.last_test_result.ok ? 'OK' : 'failed'}` : 'never'}</dd></dl>
      <form id="keyForm" class="col mt-2"><label class="field"><span>${c2.has_user_key ? 'Update' : 'Save'} your ${label} API key</span><input type="password" name="api_key" placeholder="${esc(keyHint)}" autocomplete="off"></label><div class="flex flex-wrap"><button class="btn primary" type="submit">${c2.has_user_key ? 'Update API key' : 'Save API key'}</button>${c2.has_user_key ? raw('<button class="btn danger" type="button" id="removeKey">Remove my key</button>') : raw('')}</div></form></div>
      <div class="card"><div class="card-head"><h2>API connection status</h2></div><p class="small muted">Sends a tiny test request to ${label} using the active key and reports which model answered.</p><button class="btn" id="testBtn">Test API connection</button><div id="testOut" class="mt-2"></div></div></div>`;
    body.querySelector('#keyForm').addEventListener('submit', async (e) => { e.preventDefault(); const btn = e.target.querySelector('button[type=submit]'); setBusy(btn, true, 'Saving…'); try { const r = await api.post('/api/ai/key', formValues(e.target)); toast('API key saved (encrypted)', 'success'); render({ ...c2, ...r }); } catch (er) { toast(er.message, 'error', 7000); setBusy(btn, false); } });
    const rm = body.querySelector('#removeKey'); if (rm) rm.addEventListener('click', async () => { if (!(await confirmDialog({ title: 'Remove API key', message: 'Remove your personal API key? The server key (if configured) will be used instead.', confirmText: 'Remove', danger: true }))) return; try { const r = await api.del('/api/ai/key'); toast('Key removed', 'success'); render({ ...c2, ...r }); } catch (er) { toast(er.message, 'error'); } });
    body.querySelector('#testBtn').addEventListener('click', async (e) => { setBusy(e.target, true, 'Testing…'); const out = body.querySelector('#testOut'); try { const r = await api.post('/api/ai/test'); out.innerHTML = r.ok ? `<div class="success-box">Connected · ${esc(r.provider || '')} · model ${esc(r.model)}${r.fallback_used ? ` <span class="badge warning">fallback (primary ${esc(r.primary_model)} unavailable on this key)</span>` : ''} · ${r.latency_ms} ms · key source: ${esc(r.key_source)}</div>` : `<div class="error-box">Connection failed (${esc(r.error.code)}): ${esc(r.error.message)}</div>`; } catch (er) { out.innerHTML = `<div class="error-box">${esc(er.message)}</div>`; } setBusy(e.target, false); });
  };
  render(c);
}
...
async function status(body) {
  ...
  body.innerHTML = html`<div class="grid grid-2"><div class="card"><div class="card-head"><h2>Database status</h2>${raw(badge(d.database.ok ? 'connected' : 'unreachable', d.database.ok ? 'success' : 'danger'))}</div>
    ...
    <div class="card"><div class="card-head"><h2>Environment</h2></div><dl class="kv"><dt>Runtime</dt><dd>Node ${d.environment.node}${d.environment.vercel ? ` · Vercel (${d.environment.region || 'region n/a'})` : ' · local server'}</dd><dt>AI provider</dt><dd>${d.environment.ai_provider} · ${d.environment.ai_model}${(d.environment.ai_fallback_models || []).length ? ` (fallbacks: ${d.environment.ai_fallback_models.join(', ')})` : ''}</dd><dt>AI web research</dt><dd>${d.environment.ai_web_search ? 'on' : 'off'}</dd><dt>Server API key</dt><dd>${d.environment.server_key_configured ? 'configured' : raw('<span class="badge warning">missing</span>')}</dd><dt>Encryption key</dt><dd>${d.environment.encryption_configured ? 'configured' : raw('<span class="badge warning">missing</span>')}</dd><dt>Cron secret</dt><dd>${d.environment.cron_secret_configured ? 'configured' : raw('<span class="badge warning">missing — scheduled rotation will be rejected</span>')}</dd><dt>Business-data provider</dt><dd>${d.environment.search_provider.provider} (${d.environment.search_provider.enabled ? 'enabled' : 'not enabled'})</dd><dt>Cycle</dt><dd>${d.cycle.started ? `#${d.cycle.current_cycle_number}, next rotation ${fmtDateTime(d.cycle.next_rotation_at)}` : 'not started'}</dd></dl></div></div>
