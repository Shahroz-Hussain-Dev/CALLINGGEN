...
router.post('/leads/:id/research', async (req, res) => {
  ...
  const webSearch = (req.body || {}).web_search === undefined ? config.claude.webSearch : !!(req.body || {}).web_search;
  let result;
  if (kind === 'booking_analysis') result = await claude.generateBookingAnalysis({ userId: req.user.id, contact: detail.contact, calls: detail.call_history });
  else result = await claude.generateBusinessProfile({ userId: req.user.id, contact: detail.contact, calls: detail.call_history, previousResearch: detail.research[0] ? detail.research[0].content : null, webSearch });
  ...
  await activity.log('research_generated', { userId: req.user.id, contactId: detail.contact.id, details: { kind, model: result.model, sources: result.sources.length, usage: result.usage } });
